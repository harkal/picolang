...
function clean_nops(program) {
    ...
    while(inst.next) {
        ...
        {
            let label = inst.next.label
            inst.next = inst.next.next
            inst.next.label = label
            continue
        }
        inst = inst.next
    }
...
function make_target_labels(program) {
    ...
    while(inst) {
        if (inst.label && inst.label.slice(0,4) === 't@@@')
            delete inst.label
        inst = inst.next
    }
    ...
    inst = program
    while(inst) {
        if (inst.target && typeof(inst.target) === 'object' && inst.target.label === undefined)
        {
