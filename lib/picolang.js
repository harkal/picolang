...
function instantiate_template(node, name, arg_types) {
	...
	let inst_sym = new_symbol(get_root(node), fn_name, datatypes.FUNCTION, false)
...
let codegen_pass = {
	...
	binop_expr: (node, visit)=>{
		if (node.bin_op === token.ASSIGN) {
			...
			if (!sym) {
				sym = new_symbol(get_root(node, 'def_expr'), node.LHS.value.name, node.RHS.datatype, true)
			}
