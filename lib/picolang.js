...
function make_parser(tokens, file_name) {
	...
	function parse_binop_RHS(prec, LHS) {
		while(true) {
			...
			if(!RHS) {
				ast.error({...tok, file_name}, 'Unexpected ' + tok.type)
				return null
			}
...
function get_node_with_key_up(n, key) {
	if(n.hasOwnProperty(key)) {
		return n
	} else if (n.parent) {
		return get_node_with_key_up(n.parent, key)
	}
	return undefined
}
...
let code_emitter = {
	...
	expr_list: (node, visit) => {
		...
		if (Array.isArray(node.value)) {
			for(let i = 0 ; i < node.value.length ; i++) {
				...
				// if (node.value[i].type === 'break_stmt' ||
				// 	node.value[i].type === 'continue_stmt') {
				// 	// be.append_pop()
				// 	continue
				// }
	...
	def_expr: (node, visit)=>{
		...
		node.ret_target = make_inst(inst_type.NOP)
		...
		visit(node.value)
		be.append_inst(node.ret_target)
		be.append_store_stack(`+ ${node.prototype.args.length * 4 + 4}`, node.datatype)
		for(let i = 4 ; i < -node.local_var_stack ; i+=4) {
			be.append_pop()
		}
	...
	while_expr: (node, visit)=>{
		if (node.cond.type === 'number_literal_expr' && node.cond.value === 0) {
			return
		}
		...
		be.append_load_immediate(0) // dummy while expr value

		break_depth--
		assert(break_depth === node.break_depth)
	},
	break_stmt: (node, visit)=>{
		let exit_node = get_node_with_key_up(node, 'exit_target')
		let exit_target = exit_node.exit_target
		be.append_inst_type(inst_type.DUP)
		if(exit_target) {
			let d = break_depth - exit_node.break_depth - 2 > 0 ? 1:0
			for(let i = 0 ; i < d ; i++) {
				be.append_pop()
			}
	...
	return_expr: (node, visit)=>{
		let ret_target = get_key_up(node, 'ret_target')
		visit(node.value)
		be.append_jump(ret_target)
	},
	asm_expr: (node, visit)=> {
