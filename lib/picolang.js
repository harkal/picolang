...
let code_emitter = {
	...
	binop_expr: (node, visit)=>{
		...
		function append_logic_op(type, l, e) {
			l = l || get_label('oper_')
			e = e || get_label('oper_')
			...
			be.append_jump(e)
			be.append_load_immediate(1, datatype.INT).label = l
		...
		switch(node.bin_op) {
			...
			case token.EQL:{
				...
				append_logic_op(backend.inst_type.JEQ, l, e)
				node_code = `\tSUB${m}\n\tPOP32\n\tJEQ ${l}\n\tLOAD32 0\n\tJMP ${e}\n${l}:\n\tLOAD32 1\n${e}:`
				break;
			}
			case token.NEQ:{
				let l = get_label('oper_')
				let e = get_label('oper_')
				append_logic_op(backend.inst_type.JNE, l, e)
				node_code = `\tSUB${m}\n\tPOP32\n\tJNE ${l}\n\tLOAD32 0\n\tJMP ${e}\n${l}:\n\tLOAD32 1\n${e}:`
				break;
			}
			case token.GTR:{
				let l = get_label('oper_')
				let e = get_label('oper_')
				append_logic_op(backend.inst_type.JGT, l, e)
				node_code = `\tSUB${m}\n\tPOP32\n\tJGT ${l}\n\tLOAD32 0\n\tJMP ${e}\n${l}:\n\tLOAD32 1\n${e}:`
				break;
			}
			case token.GEQ:{
				let l = get_label('oper_')
				let e = get_label('oper_')
				append_logic_op(backend.inst_type.JGE, l, e)
				node_code = `\tSUB${m}\n\tPOP32\n\tJGE ${l}\n\tLOAD32 0\n\tJMP ${e}\n${l}:\n\tLOAD32 1\n${e}:`
				break;
			}
			case token.LSS:{
				let l = get_label('oper_')
				let e = get_label('oper_')
				append_logic_op(backend.inst_type.JLT, l, e)
				node_code = `\tSUB${m}\n\tPOP32\n\tJLT ${l}\n\tLOAD32 0\n\tJMP ${e}\n${l}:\n\tLOAD32 1\n${e}:`
				break;
			}
			case token.LEQ:{
				let l = get_label('oper_')
				let e = get_label('oper_')
				append_logic_op(backend.inst_type.JLE, l, e)
				node_code = `\tSUB${m}\n\tPOP32\n\tJLE ${l}\n\tLOAD32 0\n\tJMP ${e}\n${l}:\n\tLOAD32 1\n${e}:`
				break;
			}
		}
	...
	asm_expr: (node, visit)=> {
		...
		emit_code('\nLOAD32 0') // dummy inst will be optimized
